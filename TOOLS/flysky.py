...
startAddress =  int(sys.argv[3], 0)
endAddress =  int(sys.argv[4], 0)
targetDir = sys.argv[5]
dataFile = open(baseFile, 'rb')
dataPatchFile = open(patchFile, 'rb')
data = bytearray(dataFile.read())
patch = bytearray(dataPatchFile.read())
dataFile.close()
dataPatchFile.close()
...
for index in range(startAddress, endAddress):
    if index < len(patch):
        data[index] = patch[index]
    else:
        data[index] = 0xFF
